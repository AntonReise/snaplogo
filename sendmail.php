 ...
 */
function getTopText () {
    $message = "You received a new message sent via the contact form on snaplogo.de.";
    $message = strlen($message) > 0 ? $message : "You received a new message sent via the contact form on snaplogo.de.";
...
function getMailHeaders ($from, $replyTo, $boundary) {
    $headers = array(
        ...
        "Message-ID: <" . implode("@", [md5(uniqid(time())), "snaplogo.de"]) . ">",
        "Date: ".date("r (T)"),
        "X_Mailer: PHP/" . phpversion(),
        "X-G1i-WSB-Sendmail: 113.0.28",
    );
...
function getDebugInfo() {
    ...
    if (isValidRequester() || $debugHeader === "snaplogo.de") {
...
function processMail() {
    ...
    if (empty($response)) {
        ...
        $contactFormEmails = array_map("toLower", json_decode('["elena@snaplogo.de"]'));
        if (
